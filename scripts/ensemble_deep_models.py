...
MODEL_SPECS = [
    (
        "resnet50-pretrained",
        Path("outputs/resnet50_pretrained_optimized_full/best_model.pt"),
        "resnet50",
    ),
    (
        "convnext-tiny-pretrained",
        Path("outputs/convnext_tiny_mixup_full/best_model.pt"),
        "convnext",
    ),
]
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/deep_ensemble"))
...
def main():
    ...
    for convnext_weight in np.linspace(0.0, 1.0, 11):
        ...
        metrics = calculate_metrics(val_labels, probabilities)
        row = {
