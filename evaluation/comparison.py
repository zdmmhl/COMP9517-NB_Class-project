"""Aggregate per-model evaluation outputs into report-ready comparisons."""
...
def load_evaluation_results(
    ...
    """Load all valid per-model metric directories under an evaluation root."""
    evaluation_root = Path(evaluation_root)
    summary_rows = []
    per_class_frames = []

    for method_dir in sorted(evaluation_root.iterdir() if evaluation_root.exists() else []):
...
def write_comparison_outputs(
    ...
    """Write comparison tables and figures for every evaluated model."""
