...
|---|---:|---:|---:|---:|---:|---:|
| `classes_500` | 45/50 | 64.54% | 83.52% | 63.69% | 59.3分钟 | 9704 MiB |
...
- 主规模实验从500类增加到1,000类时，Top-1变化`-14.22`个百分点，Macro-F1变化`-14.71`个百分点。
- 从1,000类继续增加到2,500类时，Top-1变化`-23.38`个百分点，Macro-F1变化`-24.84`个百分点，下降幅度进一步扩大。
- 固定500类，仅把每类训练图片从40减到20时，Top-1变化`-8.70`个百分点；减到8张时变化`-25.64`个百分点。这证明单类样本减少本身就是主要性能瓶颈。
...
所有测试指标均与 `test_predictions_top1.csv` 重新计算的Top-1和Macro-F1核对，结果见各运行目录的 `prediction_validation.json`。
